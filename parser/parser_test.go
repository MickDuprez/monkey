...
func TestIntegerLiteralExpression(t *testing.T) {
	...
	if len(program.Statements) != 1 {
		t.Fatalf("program.Statements number incorrect. got=%d", len(program.Statements))
	}
	...
	if literal.TokenLiteral() != "5" {
		t.Errorf("literal.TokenValue not %s. got=%s", "5", literal.TokenLiteral())
	}
}
...
func TestParsingPrefixExpressions(t *testing.T) {
	...
	for _, tt := range prefixTests {
		...
		stmt, ok := program.Statements[0].(*ast.ExpressionStatement)
		if !ok {
			t.Fatalf("stmt is not a ast.ExpressionStatement. got= %T", program.Statements[0])
		}
